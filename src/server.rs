                ...
                *client.buffer_size.write().await -= read.len();
                match {
                    if meta_enabled {
                        ...
                        write_to_client(stream, &mut sent_count, metalen, &read.to_vec(), &meta_vec)
                            .await
                    } else {
...
async fn decode_stream(
    ...
    let decode_read = match decoder.decode(data, slice, length) {
        Ok(decode_read) => decode_read,
        Err(e) => {
            println!(
    ...
    let new_data: Vec<u8> = if meta_info.metaint != 0 {
        data.to_vec()
    } else {
        ...
        while position < decode_read {
            ...
            if meta_info.remaining != 0 {
                ...
                let remaining_metadata =
                    std::cmp::min(decode_read - position, size - meta_info.vec.len());
                meta_info
                    .vec
                    .extend_from_slice(&data[position..position + remaining_metadata]);
                ...
                if metadata_vec[..] == [1; 0] {
                    ...
                    if let Ok(meta_str) = std::str::from_utf8(&metadata_vec[1..cut]) {
                        ...
                        if let Some(captures) = reg.captures(meta_str) {
                            ...
                            let mut serv = arc.write().await;
                            println!(
                                "Updated relay {} metadata with title '{}' and url '{}'",
                                serv.mountpoint,
                                metadata.title.as_ref().unwrap_or(&"".to_string()),
                                metadata.url.as_ref().unwrap_or(&"".to_string())
                            );
                            serv.metadata_vec = metadata_vec;
                            serv.metadata = Some(metadata);
                        } else {
                            println!(
                                "Unknown metadata format received from relay {}: `{}`",
                                arc.read().await.mountpoint,
                                meta_str
                            );
                            arc.write().await.disconnect_flag = true;
    ...
    if !new_data.is_empty() {
        arc.read().await.stats.write().await.bytes_read += new_data.len();
        broadcast_to_clients(arc, new_data, queue_size, burst_size).await;
    }
